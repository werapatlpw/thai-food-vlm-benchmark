...
IMAGE_ROOT   = "./images"
OUTPUT_FILE  = "./benchmark_results/benchmark_results_groq.xlsx"
RESUME_FILE  = "./benchmark_results/benchmark_progress_groq.json"
MODEL        = "meta-llama/llama-4-scout-17b-16e-instruct"  # Llama4 vision บน Groq
DELAY_SEC    = 4
MAX_IMAGES   = 20  # รันครั้งละ 20 รูป resume อัตโนมัติ
# ==================
...
def run_benchmark():
    ...
    total  = len(images)
    print(f"พบ {total} รูป ({total//5} เมนู × 5 รูป)")
    ...
    if os.path.exists(RESUME_FILE):
        with open(RESUME_FILE, "r", encoding="utf-8") as f:
            results = json.load(f)
        # zfill(2) รองรับทั้ง int เก่า (7→"07") และ string ใหม่ ("07"→"07")
    ...
    for i, (cat, menu_no, img_no, path) in enumerate(images, 1):
        if (cat, menu_no, img_no) in done_keys:
            ...
            try:
                ...
                if "429" in str(e) and attempt < 2:
                    ...
                    results.append({"category": cat, "menu_no": menu_no, "img_no": img_no,
                                    ...
                                    "fat_pred": None, "carb_pred": None, "raw": str(e)})
                    break
...
def save_results(results):
    ...
    hfill = PatternFill("solid", fgColor="F55036")
